...
/*
 * These operations are for cscrub, where the buffers are always aligned
 */
static inline void put_unaligned_native(unative_t val, void *p) {
	*((unative_t*)p) = val;
}
static inline unative_t get_unaligned_native(void *p) {
	return *((unative_t*)p);
}
...
void raid6_gen_syndrome(int disks, size_t bytes, void **ptrs)
{
	...
	for ( d = 0 ; (unsigned)d < bytes ; d += NSIZE*1 ) {
